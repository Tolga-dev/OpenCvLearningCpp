...
// race conditions
// it is occurred when more than on e thread try to write or read handwrite to a particular memory block
// simultaneously.
...
namespace ex8::theory
{
    ...
    // use used multiprocessor operation to optimize the computation
    // cv::parallel_for automatically will decide how to split the computation
    class parallelConvolution : public ParallelLoopBody // parallel loop body will overrider operator
    {
    private:
        Mat m_src, &m_dst;
        Mat m_kernel;
    ...
    public:
        ...
        void operator()(const Range &range) const CV_OVERRIDE
        {
            for (int r = range.start; r < range.end; r++)
            {
                ...
                for (int k = -sz; k <= sz; k++)
                {
                    auto *sptr = const_cast<uchar *>(m_src.ptr(i + sz + k));
                    for (int l = -sz; l <= sz; l++)
                    {
    ...
    void conv_seq(Mat src, Mat &dst, Mat kernel)
    {
        ...
        // we first make an output(dst) matrix with the same size as src, and add borders
        // to src image
